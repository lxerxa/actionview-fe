...
export function setItemValue(key, id, values) {
  return asyncFuncCreator({
    constant: 'ISSUE_SET_ITEM_VALUE',
    promise: (client) => client.request({ url: '/project/' + key + '/issue/' + id, method: 'put', data: values })
  });
}
...
export function doAction(key, id, workflow_id, values, screen) {
  return asyncFuncCreator({
    constant: 'ISSUE_WORKFLOW_ACTION',
    screen: screen && true,
    promise: (client) => client.request({ url: '/project/' + key + '/issue/' + id + '/workflow/' + workflow_id, method: 'post', data: values || {} })
  });
}
