...
export function index(qs) {
  return asyncFuncCreator({
    ...
    promise: (client) => client.request({ url: '/document' + (qs ? '?' + qs : '') })
  });
}

export function more(qs) {
  return asyncFuncCreator({
    constant: 'DOCUMENT_MORE',
...
export function multiReopen(ids) {
  return asyncFuncCreator({
    constant: 'DOCUMENT_MULTI_REOPEN',
    ids,
...
export function multiCreateIndex(ids) {
  return asyncFuncCreator({
    constant: 'DOCUMENT_MULTI_CREATEINDEX',
    ids,
...
export function cleanSelectedProject() {
  return { type: 'DOCUMENT_CLEAN_SELECTED' };
}
